...
1. Move the single `foo.md` from `implemented/<kind>/` to `archived/<kind>/`; `implemented` is deliberately absent from the archive path.
2. Make no body edits. Insert only `Archived: YYYY-MM-DD` immediately below `Status: implemented`, using the archival date.
3. Re-seal the note by running the verifier's write mode (step 5). Do not translate, reformat, update facts, or repair links inside the note.
...
5. Run `pnpm run verify-archived-agent-notes --write`. Its regeneration mode re-seals the archive from the notes actually present, so a class removal or an edit is re-hashed and any removed class is reported rather than appended onto a stale baseline. Run the normal verifier afterward to confirm every seal still matches.

After the note is sealed, never edit, move, or delete it. Archived notes remain valid inbound-link targets but are historical snapshots, not authority for current behavior.
