...
const guideModules = {
  guide: 'guide',
  develop: { label: 'Development', collection: 'develop' } satisfies GuideModuleLink,
  reference: { label: 'Reference', collection: 'reference' } satisfies GuideModuleLink,
} satisfies {
  guide: 'guide'
...
function moduleNav(): DefaultTheme.NavItem[] {
  return [
    { text: guideModules.develop.label, link: landingLink(guideModules.develop.collection), activeMatch: '^/develop/' },
    { text: guideModules.reference.label, link: landingLink(guideModules.reference.collection), activeMatch: '^/reference/' },
  ]
}
...
function siteTitle(): string {
  return `<span class="dsh-lockup">${wordmark}<span class="dsh-tag">Preview</span></span>`
}

export default withMermaid({
  title: 'DeepSeek Harness',
  description: 'A plugin SDK for building agent harnesses',
  base,
  head: [
