...
describe('rewriteMarkdown', () => {
  ...
  it('selects the published route for a link to a manifest page', () => {
    const { root, pages } = fixture()
...
describe('docsPages routes', () => {
  it('redirects the site root to the quick-start page', () => {
    ...
    expect(homes.map(page => page.route).sort()).toEqual(['index.md'])
  ...
  it('publishes a unique English route for every page', () => {
    const byRoute = new Map<string, DocsPage>()
    for (const page of docsPages) {
      expect(byRoute.has(page.route), page.route).toBe(false)
      byRoute.set(page.route, page)
    }
    expect(docsPages.every(page => !page.route.startsWith('en/'))).toBe(true)
    expect(docsPages.every(page => page.source.endsWith('.md') && !page.source.includes('.zh'))).toBe(true)
    for (const page of docsPages) {
  ...
  it('indexes every subsystem page in the folder README', () => {
    const pages = globSync(join(repositoryRoot, 'docs/subsystems/*.md'))
      ...
      .filter(page => !page.includes('.zh') && page !== 'README.md')
      .sort()
  ...
  it('projects every published subsystem page from English sources', () => {
    ...
    expect(subsystemPages.every(page => page.source.endsWith('.md') && !page.source.includes('.zh'))).toBe(true)
  })

  it('publishes the Cordis core API under the reference structure', () => {
  ...
  it('projects the Cordis inherited page once from its English source', () => {
...
describe('sidebar ordering', () => {
  ...
  it('refuses a section with no declared placement', () => {
    expect(() => sectionSpec('Undeclared section'))
      .toThrow('Sidebar section "Undeclared section" has no placement.')
  })

  it('ranks guide modules ahead of the development modules', () => {
    expect(sectionSpec('SDK').index).toBeGreaterThan(sectionSpec('Guide').index)
    expect(sectionSpec('Basics').index).toBeGreaterThan(sectionSpec('SDK').index)
  })
...
describe('projectedPageContent', () => {
  ...
  it('omits the source-only body from home pages', () => {
