...
describe('archived Agent Notes', () => {
  ...
  it('accepts one complete archived note with valid headers', () => {
    expect(validateArchiveArtifacts(fixture())).toEqual([])
  })

  it('rejects invalid archive headers and unknown kinds', () => {
    ...
    const wrongKind = new Map([['mystery/2026-07-26-example.md', artifacts.get('process/2026-07-26-example.md')!]])
    expect(validateArchiveArtifacts(wrongKind).join('\n')).toMatch(/unknown Agent Note kind/)
  })
  ...
  it('extends the manifest without permitting a sealed change or removal', () => {
    ...
    expect(first.added).toEqual(['process/2026-07-26-example.md'])
