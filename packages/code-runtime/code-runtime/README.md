...
| `language` | Readonly descriptor: the source language `run` expects. `'typescript'` is the value `dsh-tools` presents and the only published backend. Informational, not gating — a consumer that generates language-specific presentation switches on it and fails loud on a language it cannot present. |
