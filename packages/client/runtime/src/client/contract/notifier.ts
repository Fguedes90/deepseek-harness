// Notifier: subscription + batched notification primitive shared by the
// Session and Workspace object layers (Session, SessionManager, Workspace,
// WorkspaceManager, ProjectionValueStore). Semantics: N markDirty calls
// collapse into one microtask flush, while N markFrameDirty calls collapse
// into one animation-frame flush; the flush rebuilds the snapshot cache
// BEFORE notifying (useSyncExternalStore requires a stable getSnapshot
// reference). With no listeners the rebuild is skipped and only the dirty
// bit is set (keeps frame storms cheap); the next getSnapshot rebuilds
// lazily.
//
...
/** Subscription + batched notification primitive (shared by the object layers). */
