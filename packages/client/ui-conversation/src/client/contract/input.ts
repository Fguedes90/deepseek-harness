/**
 * Frozen input-machine contract — the shared cross-domain faces. Types only.
 * Three-tier visibility: business packages see InputState via the InputZone
 * currency; the scoped input events carry the mutation verbs; the
 * conversation wiring layer alone sees the full SessionInput. The machine
 * (machine.ts), the per-session shell (facade.ts), and the machine-internal
 * event/effect types stay input-domain; the surfaces below are the contract
 * faces other domains and the wiring layer import.
 */
import type { ClientContext, SnapshotStore } from '@deepseek-ai/dsh-client-runtime/client'
import type { Branded } from '@deepseek-ai/dsh-brand'
import type {
  ArbitrateKey, ArbitrateOutcome, CommandClaim,
  ReferenceInsert, TokenSpan,
