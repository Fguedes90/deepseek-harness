...
The shared wire protocol for the DeepSeek Harness SDK runtime: one newline-delimited JSON-RPC 2.0 transport class plus the named request, result, and notification types both wire ends speak. The package root enumerates the protocol consumer interface; source modules are not exported as deep imports. The server side is the [`dsh-sdk-jsonrpc-server`](../server/README.md) plugin; the client is [`dsh-sdk-client`](../client/README.md) (TypeScript). A pure library — no plugin, no Config, no registration.
...
- **Server→client requests are dead capability** — the transport supports them, but the server never sends one.
