 ...
 * notifications out to subscriptions, and tears the child down to
 * quiescence through a private EOF → SIGTERM → SIGKILL ladder. This client
 * runs OUTSIDE any harness context, so it spawns directly rather than
 * through the `dsh-subprocess` service — the seam's documented exception
 * for SDK-managed transports.
 *
