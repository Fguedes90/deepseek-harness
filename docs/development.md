...
- `pre-merge-commit` verifies staged archived Agent Notes before Git creates an automatic merge commit.
...
### Fence info-strings and the `ts ignore-check` ceiling

`doc-typecheck` never silently skips a fence. A fenced block is either a recognized `ts*` variant (compiled, or owned by the independent verifier named for its kind), or one of the allowlisted non-TypeScript languages `yaml`, `json`, `jsonc`, `markdown`, `text`, `sh`, `js`, `mermaid`, `toml`, `sql`, or a bare (unlabelled) fence. Any other info-string fails the gate naming its file and line, so a typo (` ```tsx `, ` ```TS `) or a new language needs an explicit decision rather than implicit silence. Spellings are canonical: use `markdown`, not `md`, and `ts`, not `typescript` — the non-canonical forms are deliberately absent from the table and re-introducing them is caught.

`ts ignore-check` is the opt-out for a partial sketch that cannot compile. Its count is ratcheted by `IGNORE_CEILING` in `scripts/doc-typecheck.ts`: the count must equal the ceiling. Above it, the gate demands making blocks compile (convert to `ts` and fix what keeps them sketches) or deleting them; below it, the gate demands lowering the ceiling in the same change, mirroring the `verify-doc-budgets` ratchet so the opt-out number only shrinks and never accrues as credit. Prefer converting a sketch to a compilable `ts` block (adding `export {}` to make a bare declaration block a module, or `declare`-ing a small elided helper) over accumulating opt-out.
