...
- **Coverage debt** (`coverage-debt`, `coverage-debt-ratchet`): sub-100% files cannot sit in the main gate, so they are excluded there and locked at measured values by the debt lane, which fails naming a file that drops ([Agent Note](../.agents/notes/proposed/testing/2026-08-17-measured-and-ratcheted-coverage-debt.md)).
